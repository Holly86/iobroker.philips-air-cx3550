...
class AirPurifier extends EventEmitter {
    ...
    constructor(deviceIp, options, adapter) {
        ...
        this.aliveTimeout = parseInt(options.aliveTimeout, 10) || 60000;
        this.reconnectInterval = parseInt(options.reconnectInterval, 10) || 180000;
        this.subscribeTimeout = Math.max(parseInt(options.subscribeTimeout, 10) || 180000, this.aliveTimeout);
        this.minReconnectInterval = parseInt(options.minReconnectInterval, 10) || 180000;
        this._lastPlaintextControlPayload = null;
        ...
        if (this.reconnectInterval < this.minReconnectInterval) {
            this.adapter.log.warn(
                `Reconnect interval (${this.reconnectInterval} ms) is lower than Minimum CoAP reconnect interval ` +
                    `(${this.minReconnectInterval} ms); using ${this.minReconnectInterval} ms to avoid hammering the device.`,
            );
            this.reconnectInterval = this.minReconnectInterval;
        }
        this.emit(
            'debug',
            `CoAP timing: aliveTimeout=${this.aliveTimeout}ms, subscribeTimeout=${this.subscribeTimeout}ms, ` +
                `reconnectInterval=${this.reconnectInterval}ms, staleTimeout=${Math.max(this.aliveTimeout * 10, 120000)}ms`,
        );
    ...
    async sync() {
        const syncRequest = [0, 0, 0, 0]
            ...
            .join('')
            .toUpperCase();
        this.emit('debug', `Sync request counter seed: ${syncRequest}`);
        this.clientKey = (await this._post('/sys/dev/sync', syncRequest)).toString();
        this.emit('debug', `Sync response client key: ${this.clientKey}`);
    }
    ...
    _post(path, payload) {
        return new Promise((resolve, reject) => {
            ...
            const payloadLength = payload ? Buffer.byteLength(String(payload)) : 0;
            this.emit('debug', `POST ${path} to ${this.deviceIp}, payloadLength=${payloadLength}`);
    ...
    _subscribeOnStatus() {
        return new Promise((resolve, reject) => {
            ...
            const timer = this.adapter.setTimeout(() => {
                ...
                this.emit(
                    'debug',
                    `No status from ${this.deviceIp} after subscribe within ${Math.round(this.subscribeTimeout / 1000)}s; keeping observe stream open`,
                );
                settle(resolve);
                ...
                res.on('data', chunk => {
                    ...
                    decryptPromise.then(status => {
                        ...
                        try {
                            ...
                            this.emit(
                                'error',
                                `Cannot parse status from ${this.deviceIp}: ${err && err.message ? err.message : err}; raw=${status}`,
                            );
                            return;
                        }
                        ...
                        this.emit('warn', `Ignoring undecryptable status packet from ${this.deviceIp}: ${err && err.message ? err.message : err}`);
                    });
    ...
    decryptPayload(encryptedData) {
        ...
        if (!/^[0-9A-Fa-f]+$/.test(encryptedDataStr) || encryptedDataStr.length < 72) {
            throw new Error(
                `Unexpected encrypted payload format: length=${encryptedDataStr.length}, head=${encryptedDataStr.substring(0, 16)}`,
            );
        }
        ...
        try {
            ...
            throw new Error(
                `AES decrypt failed: ${err && err.message ? err.message : err}; ` +
                    `payloadLength=${encryptedDataStr.length}, counter=${encodedCounter}`,
            );
        }
    ...
    encryptPayload(payload) {
        ...
        if (typeof payload === 'object') {
            this._lastPlaintextControlPayload = payload;
        ...
        const finalPayload = this.clientKey + encrypted + hash;
     ...
     */
    control(settings) {
        const rawDesired = buildControlPayload(settings);
        const payload = {
            state: {
                desired: {
                    CommandType: 'app',
        ...
        this.emit('debug', `CX3550/control settings=${JSON.stringify(settings)} rawDesired=${JSON.stringify(rawDesired)}`);
